                    ...
                    <tbody>
                        @foreach ($pagorentas as $index => $p)
                            <tr>
                                ...
                                <td>
                                  @if($p->estado==1)
                                  <label for="" class="label-primary col-lg-12">Pendiente</label>
                                  @else
                                  <label for="" class="label-success col-lg-12">Pagado</label>
                                  @endif
                                </td>
                                <td>
                                    @if($p->estado == 1)
                                    ...
                                    @else
                                    <a href="{{url('reportestesoreria/pagorenta/'.$p->id)}}" class="btn btn-info vista_previa">Imprimir</a>
                                    @endif
                                </td>
                            </tr>
                        @endforeach
...
<script type="">
    $(document).ready(function(e){
        ...
        $(document).on('click','#botoncito',function(e){
            ...
            swal({
          title: '¿Está seguro?',
          text: "¿Desea realizar el pago de la impuesto/renta al proveedor o empleado?",
          type: 'warning',
