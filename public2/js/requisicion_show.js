$(function(e){
    ...
    $(document).on("click","#add_formaPago",function(e){
      e.preventDefault();
      $("#modal_formaPago").modal("show");
    });

    /*submit forma de pago */
    $(document).on("submit","#form_formapago",function(e){
      e.preventDefault();
      var form = $("#form_formapago").serialize();
      modal_cargando();
      $.ajax({
        url:'/formapagos',
        type:'post',
        ...
        success: function(response){
          swal.closeModal();
          if(response.success){
            toastr.success('Forma de pago registrada con éxito');
            $("#formapago").append('<option selected value="'+response.formapago.id+'">'+response.formapago.nombre+'</option>');
      ...
      $(document).on("submit","#form_detalle",function(e){
        e.preventDefault();
      ...
      ///enviar la requisicion a UACI
      $(document).on("click","#enviar_requisicion",function(e){
        e.preventDefault();
        var id = $(this).attr('data-id');
        swal({
          title: '¿Está seguro?',
          text: "¿Desea enviar esta requisición a UACI?",
          type: 'warning',
          showCancelButton: true,
          confirmButtonColor: '#3085d6',
          cancelButtonColor: '#d33',
          ...
          if (result.value) {
            $.ajax({
              url:'/requisiciones/enviar/'+id,
              type:'GET',
              dataType:'json',
              success:function(response){
                if(response.success){
                  info(id);
                  toastr.success("Requisicion enviada a UACI");
                }else{
      ...
      $(document).on("click","#aprobar_requisicion", function(e){
        ...
        if(valid){
          var cuenta_id = $(".cuenta_elid").val();
          if(cuenta_id!=''){
            $("#modal_confirmacion").modal("show");
            $("#modal_aprobar_requisicion").modal("hide");
            $("#lacuentaid").val(cuenta_id);
          }else{
      ...
      /* autorizar la requision */
      $(document).on("submit","#form_confirmacion",function(e){
        e.preventDefault();
        var datos=$("#form_confirmacion").serialize();
